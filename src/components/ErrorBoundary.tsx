// AI CoAudS - Error Boundary Component
...
export class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('[ai-coauds] Error caught by boundary:', error, errorInfo);
    const captured: CapturedError = {
      error,
        ...
        <div className="min-h-screen flex items-center justify-center bg-ink-950 p-4">
          <div className="max-w-2xl w-full panel border-rosex/40 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="font-display text-xl font-bold text-ink-100">Application Error</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm font-mono text-rosex">{this.state.error?.name}</p>
              </div>

              <div>
                <p className="text-xs font-mono text-ink-500 mb-1">Message</p>
                <p className="text-sm text-ink-200">{this.state.error?.message}</p>
              </div>
